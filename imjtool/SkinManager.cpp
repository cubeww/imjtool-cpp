...
SkinPackage::SkinPackage(string name)
{
	auto addObject = [&](int index, string filename, int xo, int yo, int xnum = 1, int ynum = 1, float speed = NAN, bool tile = true)
	{
		...
		{
			objects[name] = make_shared<SkinObject>(filename, xo, yo, xnum, ynum, speed, tile);
		}
	};
	...
	else
	{
		...
		auto path = "textures/"s;
		addObject(GetIndex(SpikeUp), path + "spike_up.png", 0, 0, 1, 1, 0);
		addObject(GetIndex(SpikeDown), path + "spike_down.png", 0, 0, 1, 1, 0);
		addObject(GetIndex(SpikeLeft), path + "spike_left.png", 0, 0, 1, 1, 0);
		addObject(GetIndex(SpikeRight), path + "spike_right.png", 0, 0, 1, 1, 0);

		addObject(GetIndex(MiniSpikeUp), path + "mini_spike_up.png", 0, 0, 1, 1, 0);
		addObject(GetIndex(MiniSpikeDown), path + "mini_spike_down.png", 0, 0, 1, 1, 0);
		addObject(GetIndex(MiniSpikeLeft), path + "mini_spike_left.png", 0, 0, 1, 1, 0);
		addObject(GetIndex(MiniSpikeRight), path + "mini_spike_right.png", 0, 0, 1, 1, 0);

		addObject(GetIndex(Apple), path + "apple.png", 10, 12, 2, 1);
		addObject(GetIndex(KillerBlock), path + "killer_block.png", 0, 0);

		addObject(GetIndex(Block), path + "block.png", 0, 0);
		addObject(GetIndex(MiniBlock), path + "mini_block.png", 0, 0);
		addObject(GetIndex(BulletBlocker), path + "bullet_blocker.png", 0, 0);
		addObject(GetIndex(Platform), path + "platform.png", 0, 0);

		addObject(GetIndex(WalljumpL), path + "walljump_l.png", 0, 0);
		addObject(GetIndex(WalljumpR), path + "walljump_r.png", 0, 0);
		addObject(GetIndex(Water), path + "water.png", 0, 0);
		addObject(GetIndex(Water2), path + "water2.png", 0, 0);
		addObject(GetIndex(Water3), path + "water3.png", 0, 0);

		addObject(GetIndex(Warp), path + "warp.png", 0, 0);
		addObject(GetIndex(PlayerStart), path + "player_start.png", 0, 0);
		addObject(GetIndex(JumpRefresher), path + "jump_refresher.png", 15, 15);

		addObject(GetIndex(Save), path + "save.png", 0, 0, 2, 1);

		addObject(GetIndex(Bg), path + "bg.png", 0, 0, 1, 1, 0, false);
	}
}
...
void SkinManager::apply(string name)
{
	apply(make_shared<SkinPackage>(name));
}
